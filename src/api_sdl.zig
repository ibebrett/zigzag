...
pub const ApiSDL = struct {
    ...
    pub fn btn(self: ApiSDL, button: Button) bool {
        return switch (button) {
    ...
    pub fn camera(self: *ApiSDL, camera_x: f32, camera_y: f32) void {
        self.camera_x = camera_x;
        self.camera_y = camera_y;
    }

    pub fn map(self: ApiSDL, celx: u32, cely: u32, sx: f32, sy: f32, celw: u32, celh: u32, layer: u32) void {
    ...
    pub fn mset(self: *ApiSDL, x: u32, y: u32, tile: u8, layer: u32) void {
        const offset = 256 * 256 * layer;
        self.map_data[offset + x + 256 * y] = tile;
    }

    pub fn mget(self: *ApiSDL, x: u32, y: u32, layer: u32) u8 {
        const offset = 256 * 256 * layer;
        return self.map_data[offset + x + 256 * y];
    }

    // Internal Use
    pub fn update_input(self: *ApiSDL, input_state: InputState) void {
        self.input_state = input_state;
    }

    fn transform(self: ApiSDL, x: *f32, y: *f32) void {
        x.* = x.* - self.camera_x;
        y.* = y.* - self.camera_y;
    }
};
